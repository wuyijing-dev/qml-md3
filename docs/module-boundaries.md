...
2. Popups use **`Md3OverlayHost`** (`contentItem` / `mapToOverlay` / `ensureHostParent`) with optional `overlayWindow`; do not re-implement `Window.window` walks in each control.
