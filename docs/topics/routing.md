...
页面可继承 `Md3Page`（或自行声明 `md3HostWindow` / `md3RouteParams` / `md3GoBack` / `md3PushRoute`），由 `Md3PageHost` 注入，避免 `while (parent)` 或 duck-type `Window.window`（见 [module-boundaries.md](../guides/module-boundaries.md)）。
