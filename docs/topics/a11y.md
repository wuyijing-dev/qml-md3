...
| `Md3ApplicationWindow.defaultShowFocusRings` | QSettings 缺省时的初始值（鼠标为主的桌面应用常设 `false`） |
| `Md3Theme.textScale` | Scale type |
| `Md3Accessibility.announce` / `announceSuccess` / `announceError` | Live region messages |

`Md3FocusRing` 的 `visible` 已与 `Md3Accessibility.showFocusRings` 与门。鼠标点击聚焦不应单独强行点亮环（SearchBar / TextField 使用键盘 `focusReason`）。
...
```powershell
python scripts/checks/check_a11y_qml.py --json scripts/checks/out/a11y-scan.json --strict
python scripts/checks/check_qstr_coverage.py --json scripts/checks/out/i18n-scan.json --strict
Copy-Item scripts/checks/out/a11y-scan.json gallery/data/a11y-scan.json
```

Heuristic only. Current snapshot should report **0** Accessible gaps after rebuilding Gallery (`qrc:/gallery/data/a11y-scan.json`).
