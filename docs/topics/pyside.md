...
| Platform wheels on GitHub Release | **Optional** | Prefer `git+…@v1.1.4#subdirectory=python`; shared zip via `md3qml install --version 1.1.4` when attached to the Release |
...
Pin the same **`v1.1.4`** tag for the shared Md3 build and the Python host (see [integration.md](../getting-started/integration.md#lock-a-version-for-your-product-recommended)).
## Support matrix
...
### QML-only vs host-callable (v1.1.4)
