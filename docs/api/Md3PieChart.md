...
Pie / donut chart with hover probe (slice value + percent). Slices drawn on one Canvas (no per-slice Shape Repeater).
...
| `showPercentLabels` | `bool` | `true` | read/write | `Md3PieChart` | Show Percent Labels. |
| `startAngle` | `real` | `-90` | read/write | `Md3PieChart` | degrees, 0 = 3 o'clock |
| `cx` | `real` | `width / 2` | readonly | `Md3PieChart` | Cx. |
| `cy` | `real` | `height / 2` | readonly | `Md3PieChart` | Cy. |
| `outerR` | `real` | `Math.max(8, Math.min(width, height) / 2 - contentPadding)` | readonly | `Md3PieChart` | Outer R. |
| `innerR` | `real` | `outerR * Math.min(0.92, Math.max(0, innerRatio))` | readonly | `Md3PieChart` | Inner R. |
...
|--------|---------|------------|-------------|
| `rebuild()` | `—` | `Md3PieChart` | Rebuild. |
| `nudgeProbe(delta)` | `—` | `Md3PieChart` | Nudge Probe. |
