...
| Method | How |
|--------|-----|
| **Git tag / submodule** | `git checkout v1.1.4` or submodule at `v1.1.4` |
| **FetchContent** | `GIT_TAG v1.1.4` (annotated tag) |
| **add_subdirectory** | Clone/copy the tree at `v1.1.4`, then `add_subdirectory(...)` |
| **Packaged `./Md3`** | Build/install from that tag; keep the prefix in VCS or CI cache |
| **Python** | `pip install "git+…@v1.1.4#subdirectory=python[pyside6]"` then `md3qml install --version 1.1.4` when using Release zips |
| **Rust** | Depend on `rust/md3qml` from a checkout at `v1.1.4`; set `MD3_PREFIX` to a shared build from the same tag |

Current lock tag: **`v1.1.4`** (see [CHANGELOG](https://github.com/wuyijing-dev/qml-md3/blob/main/CHANGELOG.md)). Upgrade only when you choose a newer tag.
...
**Product contract:** pin **`v1.1.4`**, build/install shared Md3 from that tag, put UX in QML. Hosts only bootstrap + optional system helpers. Do not expect Rust C ABI to grow WindowHelper parity.
