...
14. [x] **性能（外观不变）**：按钮 clip FBO 按需；Shadow/Skeleton/Carousel 场景门控；disabled 色缓存；LiquidGlass 默认非 live；布局事件化；**DataTable TableView 行列虚拟化**；TreeView；PageHost prefetch/sparse；Form 事件驱动；Chart/Gauge 门控 — 见 [performance.md](../topics/performance.md)
