...
14. [x] **性能（外观不变）**：按钮 clip FBO 按需；Shadow/Skeleton/Carousel 场景门控；disabled 色缓存；LiquidGlass 默认非 live；布局事件化；**DataTable TableView 行列虚拟化**；TreeView；PageHost prefetch/sparse；Form 事件驱动；Chart/Gauge 门控；**`md3PageActive` + DeferredSection 离页卸重**；**PageActivityGate 表格/列表/代码块/Sparkline/选择器离页清重**；Gallery L1≤3 + L2 近邻 + Window 平台面板 Loader — 见 [performance.md](../topics/performance.md)
