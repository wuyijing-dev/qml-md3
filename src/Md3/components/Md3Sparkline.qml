...
Item {
    ...
    /// Drop Canvas FBO while page is off-display (shell size stays).
    ...
    function _requestPaint() {
        if (pageGate.contentActive && canvasLoader.item)
            canvasLoader.item.requestPaint()
    }

    onValuesChanged: _requestPaint()
    onWidthChanged: _requestPaint()
    onHeightChanged: _requestPaint()
    onStrokeChanged: _requestPaint()
    onFillChanged: _requestPaint()
    onShowAreaChanged: _requestPaint()
    onShowLastDotChanged: _requestPaint()
